...
static char RCSid[] = "$Id: show.c,v 1.1 1999/06/25 06:11:56 mark Exp mark $";
...
#define ADD_SYNTAX_LINE_OUTPUT(line,highlight,length) {        \
                       int l = length;                         \
                       CHARTYPE *src;                          \
                       chtype *dest,*highl;            \
...
/***********************************************************************/
{
 ...
 INIT_LINE_OUTPUT(win,line);
 if (linelength)
    ADD_SYNTAX_LINE_OUTPUT(str,high,linelength);
 if ((linelength = width - linelength) != 0)
    FILL_LINE_OUTPUT(' ',linelength,colour);
 END_LINE_OUTPUT();
}
...
/*--------------------------- local data ------------------------------*/
 ...
 CHARTYPE buffer[120]; /* should be large enough for very long values */
    ...
    if (screen_view->position_status)
      {
       ...
       max_name = (screen[scrno].screen_cols-47);
      }
    else
      {
       if (compatible_look == COMPAT_XEDIT)
       ...
       else
         {
          sprintf((DEFCHAR *)buffer,"Size=%-5lu Alt=%u,%u",
                                    ...
                                    screen_file->autosave_alt,
                                    screen_file->save_alt);
          max_name = (screen[scrno].screen_cols-26);
         }
      }
   }
/*---------------------------------------------------------------------*/
 ...
 switch(screen_file->pseudo_file)
   {
    ...
    default:
         if (screen_file->display_actual_filename)
           ...
           {
            strcpy((DEFCHAR *)display_path,(DEFCHAR *)screen_file->actualfname);
           }
         break;
   }
#endif
...
/*---------------------------------------------------------------------*/
 ...
 memset((DEFCHAR *)linebuf+9,' ',COLS-8);
/*---------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------*/
    ...
    {
       sprintf(buffer,"'   %s'%s",get_prefix_command(CURRENT_FILE->first_ppc->ppc_cmd_idx),
                                   " pending...");
    }
    else
 ...
 if (STATAREA_OFFSET+length < COLS-27)
    memset((DEFCHAR *)linebuf+STATAREA_OFFSET+length,' ',(COLS-27)-STATAREA_OFFSET+length);
 ...
 if (HEXDISPLAYx)
   {
    ...
    sprintf((DEFCHAR*)linebuf+(COLS-19),"'%c'=%02X/%03d  ",
                       (unsigned char) ((key == 0) ? ' ' : key),key,key);
   }
  else
   strcpy((DEFCHAR*)linebuf+(COLS-19),"            ");
/*---------------------------------------------------------------------*/
/* Display colour setting.                                             */
/*---------------------------------------------------------------------*/
#ifdef A_COLOR
 linebuf[(COLS-7)] = (colour_support) ? 'C' : 'c';
#else
 linebuf[(COLS-7)] = 'M';
#endif
/*---------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------*/
 linebuf[(COLS-6)] = (rexx_support) ? REXX_INT_CHAR : ' ';
/*---------------------------------------------------------------------*/
/* Display INSERTMODE toggle.                                          */
/*---------------------------------------------------------------------*/
 strcpy((DEFCHAR*)linebuf + (COLS-5),(INSERTMODEx) ? " Ins" : "    ");
/*---------------------------------------------------------------------*/
...
/***********************************************************************/
{
/* BE CAREFUL! This function and his friend build_lines above
 * should always be changed in conjunction. EVER!
...
/* If the current line is the scale or tab line...                     */
/*---------------------------------------------------------------------*/
    if (scurr->line_type & (LINE_TABLINE | LINE_SCALE))
    {
       if (is_prefix_on)
       {
          scurr->prefix[0] = '\0';
...
/*---------------------------------------------------------------------*/
    ...
    {
       if (direction == DIRECTION_FORWARD)
         set_prefix_contents(scrno,scurr->current,start_row,cline);
       else
         set_prefix_contents(scrno,scurr->current->next,start_row,cline);
       scurr->normal_colour = attr_shadow;
       start_row += direction;
       scurr += direction;
       rows--;
...
/*---------------------------------------------------------------------*/
 ...
 || (vcol + cols - 1 < other_start_col))
 {
    ...
    && (!current->highlight))
    {
       ADD_SYNTAX_LINE_OUTPUT(linebuf,high,cols-blanks_after_length);
 ...
 {   /* something must be displayed in another colour.
    ...
    if (vcol < other_start_col)
    {
       ...
       if (current->is_highlighting)
       {
          ADD_SYNTAX_LINE_OUTPUT(line,high,other_start_col);
    ...
    if (cols)
    {
       ...
       {
#if 0
          ADD_SYNTAX_LINE_OUTPUT(line,high,cols-blanks_after_length);
          FILL_LINE_OUTPUT(' ',blanks_after_length,normal);
#else
          ADD_SYNTAX_LINE_OUTPUT(line,high,cols);
#endif
       }
       else
       {
...
/*  && !blank_field(curr->pre->ppc_command))*/    /* ... and not blank */
      ...
      {
       scurr->prefix_colour = set_colour(screen_file->attr+ATTR_PREFIX);
    ...
    */
    scurr->gap[0] = '\0';
    scurr->gap_colour = set_colour(screen_file->attr+ATTR_GAP);
   }
#ifdef THE_TRACE
 trace_return();
#endif
