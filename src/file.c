...
static short write_line(char*, length_t, FILE*, short);
static short write_char(char, FILE*);

LINE* dir_first_line = NULL;
LINE* dir_last_line = NULL;
line_t dir_number_lines = 0L;

static short process_file_attributes(int restore_attributes, FILE_DETAILS* cf, char* filename) {
  // deprecate
...
short get_file(char* filename) {
   ...
   * Set a temporary flag here for REXXOUT and KEY files; a DIR.DIR file is determined below.
   */
  if (strcmp(dir_filename, sp_fname) == 0) {
    pseudo_file = PSEUDO_DIR;
  } else if (strcmp(rexxoutname, sp_fname) == 0) {
    pseudo_file = PSEUDO_REXX;
  } else if (strcmp(keyfilename, sp_fname) == 0) {
    pseudo_file = PSEUDO_KEY;
  }
  /*
   * If this is the first file to be edited, don't check to see if the file is already in the ring.
   * Obvious hey!
   */
  if (CURRENT_VIEW == (VIEW_DETAILS*) NULL) {  /* no files in ring yet */
     ...
     * If the file is there and it is DIR.DIR, QQUIT out of it otherwise set the current pointer to it and exit.
  ...
  /*
   * Increment the number of files in storage here, so that if there are any problems with reading the file,
   ...
   */
  sp_path_len = strlen(sp_path);
  sp_fname_len = strlen(sp_fname);
  /*
   * Copy the filename and path strings split up at the start of the function.
   */
  if ((CURRENT_FILE->fname = (char*) malloc(sp_fname_len + 1)) == NULL) {
    free_view_memory(TRUE, TRUE);
    display_error(30, "", FALSE);
    return (RC_OUT_OF_MEMORY);
  }
  ...
  if ((CURRENT_FILE->fpath = (char*) malloc(sp_path_len + 1)) == NULL) {
    free_view_memory(TRUE, TRUE);
    display_error(30, "", FALSE);
    return (RC_OUT_OF_MEMORY);
  }
  ...
  if ((CURRENT_FILE->actualfname = (char*) malloc(strlen(filename) + 1)) == NULL) {
    free_view_memory(TRUE, TRUE);
    display_error(30, "", FALSE);
    return (RC_OUT_OF_MEMORY);
  }
  strcpy(CURRENT_FILE->actualfname, filename);
  if ((CURRENT_FILE->efileid = (char*) malloc(sp_path_len + sp_fname_len + 1)) == NULL) {
    free_view_memory(TRUE, TRUE);
    display_error(30, "", FALSE);
    return (RC_OUT_OF_MEMORY);
  }
  strcpy(CURRENT_FILE->efileid, sp_path);
  strcat(CURRENT_FILE->efileid, sp_fname);
  /*
   * If we have a "real" file, generate the filename to be used in AUTOSAVE.
   */
  if (!CURRENT_FILE->pseudo_file) {
    if ((CURRENT_FILE->autosave_fname = (char*) malloc(sp_path_len + sp_fname_len + 7)) == NULL) {
      free_view_memory(TRUE, TRUE);
      display_error(30, "", FALSE);
      return (RC_OUT_OF_MEMORY);
    }
    new_filename(sp_path, sp_fname, CURRENT_FILE->autosave_fname, ".aus");
  }
  ...
  /*
   * For PSEUDO files, point the first and last line pointers to the already allocated LINE* linked list...
   */
  ...
  switch (rc) {
    ...
    case THE_FILE_EXISTS:
      if (file_writable(work_filename)) {
        ...
        if (file_readable(work_filename)) {
          ...
          display_error(14, work_filename, FALSE);
        } else {
    ...
    case THE_FILE_NAME_TOO_LONG:
      display_error(8, "- file name too long", TRUE);
      free_view_memory(TRUE, TRUE);
      return (RC_ACCESS_DENIED);
      break;
    ...
    default:
      ...
      display_error(20, work_filename, TRUE);
      break;
  }
  /*
   ...
   */
  if ((CURRENT_FILE->first_line = add_LINE(CURRENT_FILE->first_line, NULL, TOP_OF_FILE, strlen(TOP_OF_FILE), 0, FALSE)) == NULL) {
   ...
   */
  if ((CURRENT_FILE->last_line = add_LINE(CURRENT_FILE->first_line, curr, BOTTOM_OF_FILE, strlen(BOTTOM_OF_FILE), 0, FALSE)) == NULL) {
    free_view_memory(TRUE, TRUE);
    return (RC_OUT_OF_MEMORY);
  }
   ...
   * We need to save the current setting of scope so that it can be changed to ALL so that every line will be expanded.
  ...
  if (TABI_ONx) {
    ...
    rc = execute_expand_compress("ALL", TRUE, FALSE, FALSE, FALSE);
...
LINE* read_file(FILE* fp, LINE* curr, char* filename, line_t fromline, line_t numlines, bool called_from_get_command) {
  ...
  for (;;) {
    line_start = 0;
    chars_read = (length_t) fread(brec, sizeof(char), max_line_length, fp);
    if (chars_read + read_start > trec_len) {
      sprintf(trec, "Line %ld exceeds max. width of %ld. File: %s", total_lines_read + 1, max_line_length, filename);
    ...
    memcpy(trec + read_start, brec, sizeof(char) * chars_read);
    if (feof(fp)) {
      eof_reached = TRUE;
      if (chars_read > 0) {
    ...
    /*
     * For each character remaining from the previous read in an incomplete line and each character read from the last fread()...
     */
    for (i = read_start; i < chars_read + read_start; i++) {
      ...
      if (++total_lines_read >= fromline) {
        ...
        if (total_line_length > max_line_length) {
          sprintf(trec, "Line %ld exceeds max. width of %ld. File: %s", total_lines_read, max_line_length, filename);
          display_error(29, trec, FALSE);
          if (!called_from_get_command) {
            CURRENT_FILE->first_line = CURRENT_FILE->last_line = lll_free(CURRENT_FILE->first_line);
          }
...
LINE* read_fixed_file(FILE* fp, LINE* curr, char* filename, line_t fromline, line_t numlines) {
  LINE* temp = NULL;
  bool eof_reached = FALSE;
  length_t chars_read = 0;
  line_t total_lines_read = 0L, actual_lines_read = 0L;
  ...
  for (;;) {
    chars_read = (length_t) fread(trec, sizeof(char), display_length, fp);
    if (feof(fp)) {
      eof_reached = TRUE;
    }
    if (chars_read != 0) {
...
short save_file(FILE_DETAILS* cf, char* new_fname, bool force, line_t in_lines, line_t start_line, line_t* num_file_lines, bool append, length_t start_col, length_t end_col, bool ignore_scope, bool lines_based_on_scope, bool autosave) {
   ...
   */
  if (cf->pseudo_file && (autosave || CURRENT_FILE->save_alt == 0) && blank_field(new_fname)) {
    return (RC_OK);
  }
  ...
  if (strcmp(new_fname, "") != 0) { /* new_fname supplied */
     ...
     */
    rc = readlink(write_fname, buf, sizeof(buf));
    if (rc != (-1)) {
      memcpy(write_fname, buf, rc);
      write_fname[rc] = '\0';
    }
    /*
    ...
    if (file_exists(write_fname) == THE_FILE_EXISTS) {
      ...
      if (CURRENT_FILE->timecheck && !force) {
        stat(write_fname, &stat_buf);
        if (CURRENT_FILE->modtime != stat_buf.st_mtime) {
          display_error(138, "", FALSE);
          if (bak_filename != (char*) NULL) {
            free(bak_filename);
          }
          if (write_fname != (char*) NULL) {
      ...
      if (cf->backup != BACKUP_OFF) {
        if ((bak_filename = (char*) malloc(strlen(cf->fpath) + strlen(cf->fname) + strlen(BACKUP_SUFFIXx) + 1)) == NULL) {
          display_error(30, "", FALSE);
          if (bak_filename != (char*) NULL) {
            free(bak_filename);
          }
  ...
  if (fp == NULL) {
    display_error(8, "could not open for writing", FALSE);
    if (bak_filename != (char*) NULL) {
      free(bak_filename);
    }
    if (write_fname != (char*) NULL) {
  ...
  /*
   * Save the setting of scope_all if we are ignoring scope. ie full file is being written...
   */
  if (ignore_scope) {
      ...
      default:
        ...
        if (cf->tabsout_on) {
          ...
          for (;;) {
            ...
            while ((c = next_char(curr, &off, end_col + 1)) == ' ') {
              ...
              if ((newcol % cf->tabsout_num) == 0) {
                if ((rc = write_char('\t', fp)) == RC_DISK_FULL) {
                  break;
                }
                col = newcol;
              }
            ...
            if ((rc = write_char(c, fp)) == RC_DISK_FULL) {
              break;
            }
            col++;
          }
          ...
          if (start_col < curr->length) {
            ...
            /*
             * No characters to write, but we need to call this so we can handle trailing blanks for TRAILING_EMPTY setting properly
             */
            if ((rc = write_line("", 0, fp, CURRENT_FILE->trailing)) == RC_DISK_FULL) {
              break;
            }
          }
        }
  ...
  if (rc) {
    ...
    if (same_file) {
      if (cf->backup != BACKUP_INPLACE) {
        ...
        if (cf->disposition != FILE_NEW) {
          if (chown(write_fname, cf->uid, cf->gid)) {
            display_error(84, "ownerships", FALSE);
          }
        }
    ...
    /*
     * If a backup file is not to be kept, remove the backup file provided that there hasn't been a problem in writing the file.
     */
...
static short write_char(char chr, FILE* fp) {
  if (fputc(chr, fp) == chr && ferror(fp) == 0) {
    return (RC_OK);
  }
  clearerr(fp);
  display_error(57, "", FALSE);
  return (RC_DISK_FULL);
}

static short write_line(char* line, length_t len, FILE* fp, short trailing) {
  short rc = RC_OK;
  long newlen = len;

  /*
   * If we started THE with -u switch, then DO NOT do anything with TRAILING; force it to ON
   */
  if (display_length != 0) {
   ...
   * We always write the extra trailing blanks BEFORE the supplied string as 'trailing' is only ever a real value when the end of line string is written
   */
  if (trailing != TRAILING_ON) {
    newlen = 1 + (long) memrevne(line, ' ', len);
  }
  if (fwrite(line, sizeof(char), newlen, fp) != newlen) {
    display_error(57, "", FALSE);
    rc = RC_DISK_FULL;
  }
  switch (trailing) {
...
char* new_filename(char* ofp, char* ofn, char* nfn, char* ext) {
...
short remove_aus_file(FILE_DETAILS* cf) {
  char* aus_filename = NULL;

  if ((aus_filename = (char*) malloc(strlen(cf->fpath) + strlen(cf->fname) + 5)) == NULL) {
    display_error(30, "", FALSE);
    return (RC_OUT_OF_MEMORY);
  }
  new_filename(cf->fpath, cf->fname, aus_filename, ".aus");
...
short free_view_memory(bool free_file_lines, bool display_the_screen) {
  ...
  /*
   * Save details of the current view's prefix and cmd_line settings so that if the new view has different settings we can adjust the size and/or position of the new view's windows.
   */
  ...
  switch (scenario) {
    ...
    case 1:
       ...
       *----------------------------------------------------------
       */
      if ((save_prefix & PREFIX_LOCATION_MASK) != (CURRENT_VIEW->prefix & PREFIX_LOCATION_MASK) || save_gap != CURRENT_VIEW->prefix_gap || save_prefix_width != CURRENT_VIEW->prefix_width || save_cmd_line != CURRENT_VIEW->cmd_line) {
        set_screen_defaults();
        if (curses_started) {
    ...
    case 2:
       ...
       * (scenario 2)
       *----------------------------------------------------------
       * At this point the view associated with the "current" screen has been deleted.
       * vd_current now points to the "next" view.
       * We need to point vd_current at the view in OTHER_SCREEN so it can be deleted properly.
       *----------------------------------------------------------
       */
    ...
    case 3:
       ...
       * (scenario 3)
       *----------------------------------------------------------
       * we need to clean up everything as though there we are starting THE from the beginning,
       * because the command could be 'QQ#x newfile" or the "file" being displayed could be DIR.DIR and a new LS command issued.
       *----------------------------------------------------------
       */
      CURRENT_FILE->file_views--;
      free_file_memory(free_file_lines);
...
short free_file_memory(bool free_file_lines) {
  ...
  if (CURRENT_FILE->actualfname != NULL) {
    ...
    CURRENT_FILE->actualfname = (char*) NULL;
  }
  /*
  ...
  if (CURRENT_FILE->efileid != NULL) {
    ...
    CURRENT_FILE->efileid = (char*) NULL;
  }
  /*
  ...
  if (CURRENT_FILE->autosave_fname != NULL) {
    ...
    CURRENT_FILE->autosave_fname = (char*) NULL;
  }
  /*
  ...
  /*
   * Before decrementing the count of files, deregister the ring.number_of_files implied function.
   */
  if (rexx_support) {
    char tmp[20];

    sprintf(tmp, "ring.%ld", number_of_files);
...
short read_directory(void) {
  ...
  char tmp[50];
  int len;
  char dir_rec[MAX_FILE_NAME + 50];

  /*
   * Get all file info for the selected files into structure.
   * If no file name specified, force it to '*'.
   */
  if (strcmp(sp_fname, "") == 0) {
  ...
  /*
   * dir_path is set up here so that subsequent sos_edit commands can use the directory path as a prefix to the edit files filename.
   */
   ...
   * first_line is set to "Top of File"
   */
  if ((dir_first_line = add_LINE(dir_first_line, NULL, TOP_OF_FILE, strlen(TOP_OF_FILE), 0, FALSE)) == NULL) {
    return (RC_OUT_OF_MEMORY);
  }
  /*
   * last line is set to "Bottom of File"
   */
  if ((dir_last_line = add_LINE(dir_first_line, dir_first_line, BOTTOM_OF_FILE, strlen(BOTTOM_OF_FILE), 0, FALSE)) == NULL) {
    return (RC_OUT_OF_MEMORY);
  }
  curr = dir_first_line;
  /*
  ...
  for (dp = dpfirst; dp < dplast; dp++, dir_number_lines++) {
    len = sprintf(tmp, "%ld", dp->fsize);
    if (len > 8) {
      sprintf(tmp, "%7ldk", dp->fsize / 1024);
    }
    if (dp->lname != NULL) {
      sprintf(dir_rec, "%s%8s %s %s %s -> %s", file_attrs(dp->fattr, str_attr, dp->facl), tmp, file_date(dp, str_date), file_time(dp, str_time), dp->fname, dp->lname);
    } else {
      sprintf(dir_rec, "%s%8s %s %s %s", file_attrs(dp->fattr, str_attr, dp->facl), tmp, file_date(dp, str_date), file_time(dp, str_time), dp->fname);
    }
    if ((curr = add_LINE(dir_first_line, curr, dir_rec, strlen(dir_rec), 0, FALSE)) == NULL) {
      return (RC_OUT_OF_MEMORY);
    }
    free(dp->fname);
  }
...
VIEW_DETAILS* find_file(char* fp, char* fn) {
  ...
  while (CURRENT_VIEW != (VIEW_DETAILS*) NULL) {
    if (strcmp(CURRENT_FILE->fname, fn) == 0 && strcmp(CURRENT_FILE->fpath, fp) == 0) {
...
VIEW_DETAILS *find_pseudo_file(char file) {
  VIEW_DETAILS *cv;
...
static short process_command_line(char* profile_command_line, line_t line_number) {
  short rc = RC_OK;
  short len = 0;
  bool strip = FALSE;

  /*
   * If the first line of the macro file does not contain the comment 'NOREXX' abort further processing of the macro file.
   */
...
char* read_file_into_memory(char* filename, int *buffer_size) {
  FILE* fp = NULL;
  char* buffer = NULL;

  if ((stat(filename, &stat_buf) == 0) && S_ISDIR(stat_buf.st_mode)) {
    display_error(8, "specified file is a directory", FALSE);
    return NULL;
  }
  if ((buffer = (char*) malloc((stat_buf.st_size) * sizeof(char))) == NULL) {
    display_error(30, "", FALSE);
    return NULL;
  }
  /*
   ...
   */
  *buffer_size = fread(buffer, sizeof(char), stat_buf.st_size, fp);
  fclose(fp);
  return (buffer);
}
