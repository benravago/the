...
static char RCSid[] = "$Id: file.c,v 1.2 1999/07/25 11:11:19 mark Exp mark $";
...
/*---------------------------------------------------------------------*/
 if (strcmp((DEFCHAR *)dir_filename,(DEFCHAR *)sp_fname) == 0)
    ...
    if (strcmp((DEFCHAR *)rexxoutname,(DEFCHAR *)sp_fname) == 0)
       pseudo_file = PSEUDO_REXX;
    else
       if (strcmp((DEFCHAR *)keyfilename,(DEFCHAR *)sp_fname) == 0)
          pseudo_file = PSEUDO_KEY;
#endif
/*---------------------------------------------------------------------*/
...
#endif
 if ((aus_filename =
    (CHARTYPE *)(*the_malloc)(strlen((DEFCHAR *)cf->fpath)+strlen((DEFCHAR *)cf->fname)+5)) == NULL)
   {
    display_error(30,(CHARTYPE *)"",FALSE);
#ifdef THE_TRACE
    trace_return();
...
/*---------------------------------------------------------------------*/
/* If we still have at least one view, display the it.                 */
/*---------------------------------------------------------------------*/
 ...
 */
 ...
 {
      CHARTYPE tmp[20];
      sprintf((DEFCHAR *)tmp,"ring.%d",number_of_files);
...
/*--------------------------- processing ------------------------------*/
 ...
 trace_function("file.c:    read_file_into_memory");
   ...
   fread(buffer,sizeof(CHARTYPE),stat_buf.st_size,fp);
   fclose(fp);
#ifdef THE_TRACE
   trace_return();
#endif
