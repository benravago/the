...
short get_file(CHARTYPE * filename) {
   ...
   */
  if ((CURRENT_FILE->first_line = add_LINE(CURRENT_FILE->first_line, NULL, TOP_OF_FILE, strlen((DEFCHAR *) TOP_OF_FILE), 0, FALSE)) == NULL) {
   ...
   */
  if ((CURRENT_FILE->last_line = add_LINE(CURRENT_FILE->first_line, curr, BOTTOM_OF_FILE, strlen((DEFCHAR *) BOTTOM_OF_FILE), 0, FALSE)) == NULL) {
    free_view_memory(TRUE, TRUE);
    return (RC_OUT_OF_MEMORY);
  }
...
short save_file(FILE_DETAILS * cf, CHARTYPE * new_fname, bool force, LINETYPE in_lines, LINETYPE start_line, LINETYPE * num_file_lines, bool append, LENGTHTYPE start_col, LENGTHTYPE end_col, bool ignore_scope, bool lines_based_on_scope, bool autosave) {
...
CHARTYPE *new_filename(CHARTYPE * ofp, CHARTYPE * ofn, CHARTYPE * nfn, CHARTYPE * ext) {
...
short read_directory(void) {
  ...
  for (dp = dpfirst; dp < dplast; dp++, dir_number_lines++) {
    ...
    if (dp->lname != NULL) {
      sprintf((DEFCHAR *) dir_rec, "%s%8s %s %s %s -> %s", file_attrs(dp->fattr, str_attr, dp->facl), tmp, file_date(dp, str_date), file_time(dp, str_time), dp->fname, dp->lname);
    } else {
      sprintf((DEFCHAR *) dir_rec, "%s%8s %s %s %s", file_attrs(dp->fattr, str_attr, dp->facl), tmp, file_date(dp, str_date), file_time(dp, str_time), dp->fname);
    }
...
VIEW_DETAILS *find_file(CHARTYPE * fp, CHARTYPE * fn) {
  ...
  while (CURRENT_VIEW != (VIEW_DETAILS *) NULL) {
    if (strcmp((DEFCHAR *) CURRENT_FILE->fname, (DEFCHAR *) fn) == 0 && strcmp((DEFCHAR *) CURRENT_FILE->fpath, (DEFCHAR *) fp) == 0) {
