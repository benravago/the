...
int rc;
{
  ...
  switch (rc) {
    ...
    case Emem:
      return "Machine storage exhausted";
    case Equote:
      return "Unmatched \'/*\' or quote";
    case Enowhen:
      return "Expected WHEN/OTHERWISE";
    case Ethen:
      return "Unexpected THEN/ELSE";
    case Ewhen:
      return "Unexpected WHEN/OTHERWISE";
    case Eend:
      return "Unexpected or unmatched END";
    case Echar:
      return "Invalid character in program";
    case Enoend:
      return "Incomplete DO/SELECT/IF";
    case Ehex:
      return "Invalid binary or hexadecimal string";
    case Elabel:
      return "Label not found";
    case Eprocedure:
      return "Unexpected PROCEDURE";
    case Enothen:
      return "Expected THEN";
    case Enostring:
      return "String or symbol expected";
    case Enosymbol:
      return "Symbol expected";
    case Exend:
      return "Invalid data on end of clause";
    case Etrace:
      return "Invalid TRACE request";
    case Etrap:
      return "Invalid subkeyword found";
    case Erange:
      return "Invalid whole number";
    case Exdo:
      return "Invalid DO syntax";
    case Eleave:
      return "Invalid LEAVE or ITERATE";
    case Elong:
      return "Symbol > 250 characters";
    case Ename:
      return "Name starts with number or \'.\'";
    case Ebadexpr:
    ...
    case Eopstk:
      return "Evaluation stack overflow (> 30 pending operations)";
    case Ecall:
      return "Incorrect call to routine";
    case Enum:
      return "Bad arithmetic conversion";
    case Eoflow:
      return "Arithmetic overflow or underflow";
    case Eundef:
      return "Routine not found";
    case Enoresult:
      return "Function did not return data";
    case Ereturn:
      return "No data specified on function RETURN";
    case Exlabel:
      return "Unexpected label";
    case Esys:
      return "Failure in system service";
    case Elost:
      return "Implementation error";
    case Eincalled:
      return "Error in called routine";
    case Enovalue:
      return "No-value error";
    case Eexist:
      return "Use of an un-implemented feature!";
    case Esyntax:
      return "Syntax error";
    case Elabeldot:
    ...
    case Ercomm:
      return "Unexpected \'*/\'";
    case Emanyargs:
      return "Too many arguments (> 30)";
    case Eerror:
      return "ERROR condition occurred";
    case Efailure:
      return "FAILURE condition occurred";
    case Eerrno:
      return "Unknown error occurred during I/O";
    case Ebounds:
      return "File position was out of bounds";
    case Eseek:
      return "Reposition attempted on transient stream";
    case Eaccess:
      return "Write attempted on a read-only stream";
    case Eread:
      return "Read attempted on a write-only stream";
    case Eeof + Eerrno:
      return "End of file";
  }
...
int len, varlen;                /* which has length `len'              */
{
  ...
  if (compound) {               /* A compound symbol is being assigned to */
    ...
    oldptr = name, varlen -= ((name = 1 + strchr(name, '.')) - oldptr);
...
#define word varnamebuf         /* "word" seems a better name just now */
  ...
  while (ilen-- || !interpret && srcptr > source[lines] || wordlen || !start) {
    ...
    if (token == THEN || token == ELSE || token == OTHERWISE) {
      ...
      if (!start) {
        prgptr++[0] = 0;
        prog[stmts].sourcend = prevptr, prog[++stmts].line = prgptr, prog[stmts].source = prevptr, prog[stmts].num = (interpret ? 0 : lines), prog[stmts].related = 0;
      }
      prgptr++[0] = token, prgptr++[0] = 0;
      prog[stmts].sourcend = srcptr - 1;
      prog[++stmts].line = prgptr, prog[stmts].num = (interpret ? 0 : lines), prog[stmts].source = srcptr - 1, prog[stmts].sourcend = 0, prog[stmts].related = 0;
      token = 0;
      start = gobble = 1;
      first = last = 0;
    } else if (token) {
    ...
    if (c == ';') {
      ...
      prgptr++[0] = 0;
      prog[stmts].sourcend = srcptr - 1, prog[++stmts].line = prgptr, prog[stmts].source = srcptr, prog[stmts].sourcend = 0, prog[stmts].num = (interpret ? 0 : lines), prog[stmts].related = 0;
      start = gobble = 1;
      first = last = 0;
      continue;
    }
