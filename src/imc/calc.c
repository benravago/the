    ...
    while (lp) {                /* loop until a value has been stacked */
      ...
      switch (line[*ptr]) {
        ...
        case ',':
          die(Ecomma);
        case '(':
          (*ptr)++,             /* parenthesised expressions are stacked by */
        ...
        case '+':
          (*ptr)++,             /* for unary '+' stack code OPplus, priority 11 */
          opstack[opptr].op = OPplus, opstack[opptr++].pri = 11;
          break;
        case '-':
          (*ptr)++,             /* for unary '-' stack code OPneg, priority 11 */
          opstack[opptr].op = OPneg, opstack[opptr++].pri = 11;
          break;
        case '\\':
          (*ptr)++,             /* for '\' stack code OPnot, priority 11 */
          opstack[opptr].op = OPnot, opstack[opptr++].pri = 11;
          break;
        case '\'':             /* Quoted expression... */
        case '\"':
          quote = line[(*ptr)++], expn = *ptr;
          ...
          if (line[expn] == '(') {
            ...
            exp = delete(&n);
            if (n > maxvarname - 1) {
              die(Elong);  /* the function name is */
            }
        ...
        default:               /* stack a variable or literal or
          ...
          what[1] = 'L';
          if (t != 1) {
            /* a constant symbol. Special processing
            ...
            n = 1;
            dot = 0;            /* dot is the count of dots in the number; n is
            ...
            /* A symbol follows.  If a '(' follows the symbol, then
...
rxfncall:
              varname[n] = 0;
              /* The name is nul-terminated (it cannot */
            ...
            getvarname(line, ptr, varname, &explen, maxvarname), vg = varget(varname, explen, &expn);
            if (vg == cnull) {  /* See if novalue errors are caught */
              ...
              if ((sgstack[interplev].bits & (1 << Inovalue)) && (interact < 0 || interact + 1 != interplev)) {
                errordata = varname, die(Enovalue);
              }
              stack(varname, explen);
            } else {
              what[1] = 'V', stack(vg, expn);  /* it was found */
            }
          }
          lp = 0;
      }
    ...
    /* terminators are ';', 'EOL', ')', ',' and all `words'.  Priority 0
      ...
      (*ptr)++;                 /* Go past the op */
      for (op = 0; binops[(int)op] != ch && op < 24; op++);  /* "op" holds its index */
      if (op < 24) {
        pri = binpri[(int)op];  /* "pri" holds its priority */
      } else
        (*ptr)--,               /* The char is not a binary operator, so it must be */
    ...
    while (opstack[opptr - 1].pri >= pri && opstack[opptr - 1].pri) {
      opptr--, eworkptr = 0, binprg[(int)opstack[opptr].op] (opstack[opptr].op);     /* This does the op */
      if (intermed)
...
void stacknull() {              /* Stack a null - i.e. a value with length -1 */
  ...
  (*(int *) (cstackptr + ecstackptr)) = -1, ecstackptr += four;
}
...
char op;
{
  ...
  if ((n2 = num(&m2, &e2, &z2, &l2)) < 0) {
    die(Enum);  /* The two numbers are fetched */
  }
  ...
  if (e1 < e2) {
    n3 = n2, n2 = n1, n1 = n3, n3 = m2, m2 = m1, m1 = n3, e3 = e2, e2 = e1, e1 = e3, l3 = l2, l2 = l1, l1 = l3;  /* now e1>=e2 always */
  }
  ...
  if (m1 == m2) {               /* add two numbers; the sign of the result is m1 */
    ...
    for (i = l3 - 1; i >= 0; i--) {
      ...
      if (d2 < 0 || d2 >= l2) {
        d2 = 0;  /* corresponding digit in n2 */
      } else {
    ...
    if (c) {
      n3--, workptr[n3] = '0' + c, l3++, e3++;  /* carry to the left */
    }
...
char op;
{
  ...
  if ((n1 = num(&m1, &e1, &z1, &l1)) < 0) {
    die(Enum);  /* Get each number and delete */
  }
  ...
  for (i = l2 - 1; i >= 0; i--) {       /* Now a long multiplication */
    c = 0, d1 = workptr[n2 + i] - '0';
    for (j = l1 - 1; j >= 0; j--) {
      k = i + j + 1, d = (workptr[n1 + j] - '0') * d1 + c + workptr[n3 + k] - '0', c = d / 10, d %= 10, workptr[n3 + k] = d + '0';
    }
    workptr[n3 + i] += c;
  }
...
char op;
{
  ...
  if (op != OPdiv) {
    l3 = e1 - e2 + 1;
  }           /* For % and //, the number of digits in
  ...
  for (i = 0; i < l3; i++) {    /* loop for each digit of result */
    ...
    while (1) {
      ...
      if (z1) {
        workptr[n3 + i] += mul;
        break;
      }                         /* Exactly zero resulted */
      if (!c) {
        workptr[n3 + i] += mul;
        continue;
      }                         /* A positive value resulted */
      c = 0;
...
char op;
{
  ...
  while (pow > 0) {
    pow <<= 1, c--;  /* Get the MSB of the num into the MSB of the int */
  }
  ...
  if (pm) {                     /* The exponent was negative, so invert the number */
    mtest(cstackptr, cstacklen, ecstackptr + 2 * four, 256);
    ptr = cstackptr + ecstackptr - four, l = align(*(int *) ptr), ptr -= l,     /* ptr points to the stack entry containing the result */
          l += four,              /* l contains its whole length */
               n = four + align(1);
    for (c = l - 1; c >= 0; c--) {
      ptr[c + n] = ptr[c];  /* Make two ints-worth of space */
    }
    ptr[0] = '1', *(int *) (ptr + align(1)) = 1,        /* Store the stack entry "1" in the space */
             ecstackptr += n, bindiv(2);     /* Now divide 1 by the result. */
  }
    ...
    if (!n) {                   /* both numeric */
      if (!n && (m ^ m2)) {     /* different signs: */
        ...
        i = precision, precision = fuzz,        /* add fuzz */
                                   ...
                                   precision = i, n = num(&m, &i, &z, &l),     /* get flags, then discard the result */
                                               delete(&l);
      }
    } else {                    /* string comparison; first strip spaces */
      ecstackptr += align(len2) + four; /* restack string2 */
      strip(), ptr2 = (unsigned char *) delete(&len2),  /* Get pointers and lengths of */
...
char op;
{
  ...
  if (num(&minus, &exp, &z1, &len) < 0) {
    die(Enum);  /* Get the two numbers and */
  }
...
void strip() {                  /* Strip leading and trailing spaces from the top stack value */
  ...
  if (ptr == ptr1) {
    ...
    for (i = 0; i < len; i++, (*(ptr1++)) = (*(ptr++)));        /* else move it back */
  (*(int *) (ptr1 + toalign(len))) = len, ecstackptr += align(len);
}
...
int *minus, *exp, *zero, *len;
{
  ...
  if (len1 < 0) {
    die(Ecall);
  }                 /* doing any numeric operation whatsoever on a 'null'
  ...
  if ((ch = ptr1[0]) == '-') {
    ptr1++, len1--, (*minus) = 1;  /* Deal with leading sign */
  } else if (ch == '+') {
    ptr1++, len1--;
  }
...
int flg;
{
  ...
  if ((n = num(&minus, &exp, &zero, &len)) < 0) {
    die(Enum);  /* First get the number  */
  }
...
int *len;                       /* its position and setting "len" to its length.  The  */
{
  ...
  (*len) = *(int *) ptr;
  if (*len >= 0) {
    ptr -= align(*len), ecstackptr -= align(*len);
  } else {
...
int exp, len, minus;
{
  ...
  if (len - exp - 1 <= 2 * precision && exp < precision) {      /* stack with no exponent */
    ...
    if (exp) {                  /* Add the exponent                 */
      ptr1[len1++] = 'E', ptr1[len1++] = exp < 0 ? '-' : '+', exp = abs(exp);
      if (exp > maxexp) {
        die(Eoflow);
      }
      for (i = 1; i <= exp; i *= 10);
...
int maxlen;                     /* the amount of space allocated to varname */
{
  ...
  while (line[*ptr] == '.') {   /* Loop to interpret the qualifiers of a comp.symb. */
    ...
    switch (line[*ptr]) {       /* What kind of qualifier is it? */
      case '.':
        break;                  /* null qualifier */
      case '(':
        (*ptr)++,               /* parenthesised expression e.g. stem.(a+b) */
...
int ex;                         /* whether or not an entire expression is allowed */
{
  ...
  if (line[*ptr] == ')') {
    die(Erpar);
  }                 /* save some work by flagging extra ')'s
