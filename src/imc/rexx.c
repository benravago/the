...
/* Clean program stack and print out traceback */
  ...
  if (!errline)
    errline = sigl;             /* don't say "error in line 0" if it happened
...
/* End of control variable processing; start of numeric count processing  */
            ...
            ((struct forstack *) entry)->fornum = i, ((struct forstack *) entry)->len = sizeof(struct forstack), ((struct forstack *) entry)->type = stype = 15, epstackptr += sizeof(struct forstack) - sizeof(struct minstack);
          }
...
/* check for internal label */
  ...
  sptr->csp = oldcstackptr, sptr->ecsp = ecstackptr, sptr->csl = cstacklen, sptr->trc = trcflag, sptr->tim = timeflag, sptr->mic = microsecs, sptr->sec = secs, sptr->address1 = address1, sptr->address2 = address2, sptr->form = numform, sptr->digits =
  ...
  cstackptr = sptr->csp, ecstackptr = sptr->ecsp, cstacklen = sptr->csl, trcflag = sptr->trc, timeflag = (timeflag & 4) | (sptr->tim & 3), microsecs = sptr->mic, secs = sptr->sec, address1 = sptr->address1, address2 = sptr->address2, numform =
