...
recurse:
  ...
  switch (rc) {                 /* find out what trap has occurred, based on rc */
    ...
    case 0:
      /*    case Esys:
            case Emem: */
      bit = 0;
      break;                    /* never catch `OK' or `out of memory'
  ...
  if (catch) {
    ...
    if (stmt < 0) {
      /* If label not found, then see whether this
      ...
      if (!catch) {
        ...
        if (bit == Inotready) {
          tracestr("      +++ "), tracestr(sigdata[bit]), tracestr(": "), tracestr(message(Eerrno + lasterror));
        } else {
          tracestr("      +++ "), tracestr(message(rc));
        }
  ...
  if (prog && !ppc && rc >= 0) {        /* Error occurred while tokenising */
    ...
    if (!ptr) {                 /* default: start of the last line encountered */
      ptr = prog[stmts].source = source[lines], prog[stmts].num = lines;
    }
  ...
  if (!errline) {
    errline = sigl;
  }             /* don't say "error in line 0" if it happened
  ...
  if (rc > 0) {
    tracestr("Error "), tracenum(rc, 1);
    if (ppc < 0) {
      tracestr(" interpreting arguments: ");
    } else {
      tracestr(" running "), tracestr(fname), tracestr(", line "), tracenum(errline, 1), tracestr(": ");
    }
    tracestr(message(rc));
    if (edata) {
      tracestr(edata);
...
int delay;                      /* Whether to delay any signals */
{
  ...
  if (inherit) {
    ...
    sgstack[interplev].bits = 0, sgstack[interplev].callon = 0, sgstack[interplev].delay = 0, sgstack[interplev].type = 0, sgstack[interplev].data = 0;
  }
  ...
  while (ppc < stmts) {
    ...
    if (trcflag & Tclauses) {
      if ((c = *lineptr) == END && pstacklev)
        tmpchr = epstackptr, unpstack(),        /* at an END, print out also the DO */
                 delpstack(),        /* this un-indents the END and the DO */
                 printstmt(ppc, 0, 0),       /* whilst maintaining a consistent stack */
                 printstmt(newppc, 0, 0), pstacklev++,       /* put the deleted stack entry back. */
                 epstackptr = tmpchr;
      else {
        printstmt(ppc, 0, 0);
      }
      ...
      switch (c2 = *lineptr++) {
        ...
        case SAY:              /* With SAY, follow it with a newline */
          ...
          if (c2 == SAY && exitlist[RXSIO]) {
            ...
            exp[len - 1] = 0;
            if (exitcall(RXSIO, RXSIOSAY, &rxs) == RXEXIT_HANDLED) {
              break;
            }
            exp[len - 1] = '\n';
          }
        ...
        case DO:
          ...
          if (varname[0]) {
            ...
            while ((c = *lineptr) == TO || c == BY || c == FOR) {
              tmpchr = 1;
              if (c == TO) {
                slimit = scanning(lineptr, &tmpchr, &sllen), ilimit = slimit - cstackptr;
              } else if (c == BY) {
                sstep = scanning(lineptr, &tmpchr, &sslen), istep = sstep - cstackptr;
              } else {          /* c==FOR */
            ...
            stack(istart + cstackptr, len), varset(varname, varlen, istart + cstackptr, len),   /* var = start */
                  tmpchr = ecstackptr;    /* save the address of the following: */
            if (sllen >= 0) {
              stack(ilimit + cstackptr, sllen), sllen = 1;
            } else {
          ...
          /* End of control variable processing; start of numeric count processing  */
            ...
            ((struct forstack *) entry)->fornum = i, ((struct forstack *) entry)->len = sizeof(struct forstack), ((struct forstack *) entry)->type = stype = 15,
        ...
        case END:
          ...
          if (c == UNTIL) {
            tmpchr = 1, scanning(tmpptr, &tmpchr, &len), tmpptr += tmpchr, eworkptr = 0, len = num(&m, &e, &z, &l);
          ...
          if (!whilep && *tmpptr == WHILE) {
            ...
            if ((c = *tmpptr))  /* should be nothing after WHILE */
              delpstack(), ppc = tmpppc,        /* flag the error in the DO statement */
                                 die(Edata);
          }
          ...
          if (whilep) {
            ppc = tmpchr, delpstack();
          } else {
            lineptr = tmpptr;  /* copy the character ptr to the end */
          }
          /* of the DO clause */
          break;
        case IF:
          tmpchr = 0, scanning(lineptr, &tmpchr, &len);
          if (num(&m, &e, &z, &l) < 0) {
...
when:
        ...
        case OPTIONS:          /* Split the option into tokens and call setoption */
          tmpchr = 0, exp = scanning(lineptr, &tmpchr, &len), lineptr += tmpchr;
          while (len) {
            while (len && *exp == ' ') {
              exp++, len--;
            }
        ...
        case PARSE:
          ...
          switch (lineptr++[0]) {
            case ARG:
              ...
              break;
            case SOURCE:
              parselist[0] = psource, parselen[0] = strlen(psource);
              break;
            ...
            case LINEIN:       /* mirrors the linein() function */
              if (!(info = (struct fileinfo *) hashget(1, "stdin", &l))) {
                ...
                if (info->lastwr == 0 && (filepos = ftell(info->fp)) >= 0 && filepos != info->rdpos) {
                  info->rdpos = filepos, info->rdline = 0;  /* position has been disturbed */
                }
                clearerr(info->fp);
                if (info->lastwr) {
              ...
              // }
              parselist[0] = pull, parselen[0] = l;
              break;
            case VALUE:
              i = 0;
              if (*lineptr == WITH) {
                parselist[0] = "", parselen[0] = 0;
              } else
                while (1) {
                  tmpchr = 0, parselist[i] = scanning(lineptr, &tmpchr, &parselen[i]), lineptr += tmpchr;
                  if ((c = *lineptr) == WITH) {
                    break;
                  }
                  if (c != ',' || i == maxargs) {
                  ...
                  while (*lineptr == ',') {
                    lineptr++, parselist[++i] = "", parselen[i] = 0;
                  }
                }
              i++, lineptr++;
              if (parselen[0]) {
              ...
              break;
            case VAR:
              tmpchr = 0, getvarname(lineptr, &tmpchr, varname, &varlen, maxvarname);
              lineptr += tmpchr;
              if (varname[0] == 0) {
                die(Enosymbol);
              }
              if ((exp = varget(varname, varlen, &parselen[0])) == cnull) {
                ...
                if (sgstack[interplev].bits & (1 << Inovalue)) {
                  errordata = varname, die(Enovalue);  /* A novalue error was caught */
                }
                parselist[0] = varname, parselen[0] = strlen(varname);
              } else {
                /* Copy the variable's value to pull.  We can't
          ...
          if (*lineptr) {       /* if a template supplied, */
            tmpchr = 0, parse(parselist, parselen, up, lineptr, &tmpchr), lineptr += tmpchr;
          }
          break;
          ...
          if ((c = *lineptr) == ON || c == OFF) {       /* set or clear a trap */
            ...
            if (c == ON) {
              ...
              for (l = istart; l <= interplev; l++) {
                sgstack[l].bits &= ~(1 << i), sgstack[l].bitson &= ~(1 << i), sgstack[l].delay &= ~(1 << i), sgstack[l].callon &= ~(1 << i);
              }
            break;
          }
        ...
        case SIGNAL:
          ...
          if ((c = *lineptr) == ON || c == OFF) {       /* set or clear a trap */
            ...
            if (c == ON) {
              ...
              for (l = istart; l <= interplev; l++) {
                sgstack[l].bits |= (1 << i), sgstack[l].callon &= ~(1 << i), sgstack[l].delay &= ~(1 << i);
              }
            } else
              for (l = istart; l <= interplev; l++) {
                sgstack[l].bits &= ~(1 << i), sgstack[l].bitson &= ~(1 << i), sgstack[l].callon &= ~(1 << i), sgstack[l].delay &= ~(1 << i);
              }
            break;
...
signal:
          ...
          if (!l) {             /* the label wasn't found */
            sprintf(workptr, ": \'%s\'", signalto), errordata = workptr, die(Elabel);
          }
          /* before jumping, save current ppc in variable SIGL */
          sprintf(varname, "%d", prog[ppc].num), varset("SIGL", 4, varname, strlen(varname)), ppc = lptr[1], chkend = 0;
          break;
        case ITERATE:          /* Find the END and jump to it */
          ...
          while (1) {
            /* delete stack items until the right loop found. The
...
leaveit:               /* find the "sllen"th END and jump past it */
        ...
        case INTERPRET:        /* Get the details and call rxinterp */
          ...
          if (*anslen >= 0) {
            return exp;
          }         /* "interpret 'return x'" causes x to be returned
        ...
        case NUMERIC:          /* get parameter, and set global variable */
          ...
          if ((c = lineptr++[0]) == FORM) {
            ...
            if (!strcmp(varname, "SCIENTIFIC")) {
              numform = 0;
            } else if (!strcmp(varname, "ENGINEERING")) {
              numform = 1;
            } else {
              die(Eform);  /* invalid subkeyword */
            }
      ...
      /* The instruction starts with a printable character.  Try an
      ...
      if (rexxsymbol(c = *lineptr) == 1) {      /* the character is the */
        ...
        if (lineptr[tmpchr] == '=')     /* it is an assignment if the  */
          tmpchr++,             /* next character is '='       */
                 exp = scanning(lineptr, &tmpchr, &len), lineptr += tmpchr, varset(varname, l, exp, len);
...
int *ptr;                       /* The current character pointer positioned at the template */
{
  ...
  while (1) {                   /* loop for each template separated by commas */
    ...
    while (1) {
      if (line[*ptr] == ' ') {
        ++ * ptr;
      }               /* A space may separate the previous piece
      ...
      if (c == '(') {           /* parse expression */
        ...
        if (line[*ptr] == 'X' && !rexxsymboldot(line[*ptr + 1])) {
          stackx(srch, srchlen), (*ptr)++;
        } else if (line[*ptr] == 'B' && !rexxsymboldot(line[*ptr + 1])) {
          stackb(srch, srchlen), (*ptr)++;
        } else {
          stackq(srch, srchlen, c);
        }
        srch = delete(&srchlen);
      } else {
        /* parse numeric. c holds the sign (+,-,=) if any. Stack the
      ...
      if (srchlen == 0) {       /* The null string matches the end of the line. */
        ...
        for (k = j; k <= l - srchlen; k++) {    /* Do the search */
      ...
      if (k == l) {
        ...
        j = k + srchlen, lastexpr = k;  /* Move to end of string, but save the */
      }
    }                           /* start position */
...
int up;                         /* Whether to uppercase the value */
{
  ...
  while (listlen) {
    ...
    val += ptr;
    if (len -= ptr) {
      val++, len--;  /* absorb one space if necessary */
    }
  }
}
...
int *level;                     /* the most recent non-interpreted instruction, and       */
{
  ...
  for (ptr = pstackptr + (eptr = epstackptr); !sigl && eptr;) {
    ...
    eptr -= len;
    if (type == 14) {
      prog = ((struct interpstack *) ptr)->prg, stmts = ((struct interpstack *) ptr)->stmts, lev--;
    }
...
int lit;                        /* whether or not the name was a quoted literal (if it was,  */
{
  ...
  while (argc && isnull()) {
    argc--, delete(&w);  /* The last arg should not be null */
  }
  if (argc > maxargs) {
      ...
      if (w == 1) {             /* The file is a Rexx program, so start it */
        for (l = argc - 1; l >= 0; l--) {
          ...
          if (w >= 0) {
            ...
            rxargs[l].strptr = 0, rxargs[l].strlength = -1;
          }
        }
        rxresult.strptr = 0;
  ...
  sprintf(file, "%d", sigl), varset("SIGL", 4, file, strlen(file));     /* ("file" is unused in this case) */
  getcallargs(args, arglen, argc), oldcstackptr = cstackptr, cstackptr = allocm(100);
  sptr = (struct procstack *)   /* We now stack a program stack item... */
         pstack(11, sizeof(struct procstack2));
  sptr->csp = oldcstackptr, sptr->ecsp = ecstackptr, sptr->csl = cstacklen, sptr->trc = trcflag, sptr->tim = timeflag, sptr->mic = microsecs, sptr->sec = secs, sptr->address1 = address1,
                                                                                                                                                                      sptr->address2 = address2, sptr->form = numform, sptr->digits =
                                                                                                                                                                          precision, sptr->fuzz = fuzz;
  sptr->stmts = oldstmts, sptr->prg = oldprog;
  ...
  trcresult = oldtrcres, interplev--, oldcstackptr = cstackptr, timestamp.tv_sec = oldsec, timestamp.tv_usec = oldmic;
  while (type < 11 || type > 12) {      /* Clear up all entries until ours */
    type = unpstack(), sptr = (struct procstack *) delpstack();
  }
  cstackptr = sptr->csp, ecstackptr = sptr->ecsp, cstacklen = sptr->csl, trcflag = sptr->trc, timeflag = (timeflag & 4) | (sptr->tim & 3), microsecs = sptr->mic, secs = sptr->sec,
                                                                                                                                                                  address1 = sptr->address1, address2 = sptr->address2, numform =
                                                                                                                                                                      sptr->form, precision = sptr->digits, fuzz = sptr->fuzz;
  if (result) {
    stack(result, rlen);
  }
  free(oldcstackptr);           /* Now the result has been used, free the old stack */
  stmts = sptr->stmts, prog = (sptr->prg);
  if (type > 11) {              /* reclaim procedural variables */
    varstkptr--;
  }
  curargs = oldcarg, curarglen = oldcarglen;
  ppc = newppc;
...
int arglen[];                   /* The array of lengths of arguments */
{
  ...
  if (*rlen >= 0)
    while (type != 14) {
      delpstack();
      type = unpstack();
    } else /* otherwise all structures should be complete */ if (type != 14) {
    die(Enoend);
  }
  interplev--;
  sptr = (struct interpstack *) delpstack(), ppc = newppc, free(prog[0].source),        /* the interpreted string */
...
void on_halt() {
  /* when a halt occurs, this function is called to set
