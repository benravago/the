...
int sig;
{
  ...
  switch (sig) {
    case SIGINT:
      sigdata[Ihalt] = "SIGINT";
      putc('\n', ttyout);
      break;
    case SIGHUP:
      sigdata[Ihalt] = "SIGHUP";
      break;
    default:
...
SHVBLOCK *request;
{
  ...
  for (; request; ret |= request->shvret, request = request->shvnext) {
    ...
    switch (request->shvcode) {
      ...
      case RXSHV_SYSET:
        nextvar = 0;
        varset(name, namelen, request->shvvalue.strptr, request->shvvalue.strlength);
        break;
      case RXSHV_FETCH:
      case RXSHV_SYFET:
        nextvar = 0;
