...
short extract_macro(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_VIEW->macro, 1);
}

short extract_macroext(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (strlen((char *) macro_suffix) == 0) {
    item_values[1].value = (uchar *) macro_suffix;
  } else {
    item_values[1].value = (uchar *) macro_suffix + 1;
  }
  item_values[1].len = strlen((char *) item_values[1].value);
  return number_variables;
}

short extract_macropath(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  item_values[1].value = (uchar *) the_macro_path;
  item_values[1].len = strlen((char *) the_macro_path);
  return number_variables;
}

short extract_margins(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_modifiable_function(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_monitor(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (colour_support) {
    item_values[1].value = (uchar *) "COLOR";
    item_values[1].len = 5;
  } else {
    item_values[1].value = (uchar *) "MONO";
    item_values[1].len = 4;
  }
  item_values[2].value = (uchar *) "COLOR";
  item_values[2].len = 5;
  return number_variables;
}

short extract_mouse(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(MOUSEx, 1);
}

short extract_mouseclick(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  int mouseclick = -1;
...
short extract_msgline(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  ...
  if (CURRENT_VIEW->msgline_rows) {
    ...
    item_values[3].value = (uchar *) "*";
  }
  item_values[3].len = strlen((char *) query_num1);
  item_values[4].value = (uchar *) "OVERLAY";
  item_values[4].len = 7;
  return number_variables;
}

short extract_msgmode(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  set_on_off_value(CURRENT_VIEW->msgmode_status, 1);
  item_values[2].value = (uchar *) "LONG";
  item_values[2].len = 4;
  return number_variables;
}

short extract_nbfile(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_nbscope(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  long number_lines = 0;
...
short extract_newlines(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (CURRENT_VIEW->newline_aligned) {
    item_values[1].value = (uchar *) "ALIGNED";
    item_values[1].len = 7;
  } else {
    item_values[1].value = (uchar *) "LEFT";
    item_values[1].len = 4;
  }
  return number_variables;
}

short extract_nondisp(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_number(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_VIEW->number, 1);
}

short extract_pagewrap(short pagewrap_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(PAGEWRAPx, 1);
}

short extract_parser(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  short rc = RC_OK;
  int i = 0;
  int off = 0;
  bool found = FALSE;
...
short extract_pending(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  ...
  LINE *curr;
  long first_in_range = 0L;
  long last_in_range = CURRENT_FILE->number_lines + 1;
  ...
  for (;;) {
    ...
    switch (state) {
      case STATE_START:
        if (equal((uchar *) "block", word[i], 5)) {
          find_block = TRUE;
          state = STATE_OLDNAME;
          i++;
          break;
        }
        if (equal((uchar *) "oldname", word[i], 7)) {
          find_oldname = TRUE;
          state = STATE_NAME;
          i++;
          break;
      ...
      case STATE_TARGET1:
        ...
        break;
      case STATE_TARGET2:
        if (equal((uchar *) "*", word[i], 1)) {
          /*
           * If the second target is *,
           * then default last_in_range to be 1 more than the number of lines in the file;
           * same as if no second target was specified.
           */
  ...
  if (number_variables >= 0) {
    ...
    if (CURRENT_FILE->first_ppc == NULL) {
      ...
      for (;;) {
        ...
        /*
         * To imitate XEDIT behaviour,
         * ignore a prefix command if the line on which the prefix command has been entered is not in scope.
         */
        ...
        if (strcmp((char *) name, "*") == 0) {
          ...
          if (find_block) {
            ...
            /*
             * Go back and look for another either because we didn't find a block command,
             * or because we did, but it may not be the one with the smallest line number.
             */
            curr_ppc = curr_ppc->next;
            continue;
          } else {
            ...
            /*
             * Go back and look for another because it may not be the one with the smallest line number.
             */
            curr_ppc = curr_ppc->next;
            continue;
          }
      ...
      if (found_ppc == NULL) {
        ...
        if (found_ppc->ppc_block_command) {
          item_values[4].value = (uchar *) "BLOCK";
        } else {
          item_values[4].value = (uchar *) "";
        }
...
short extract_point(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  long true_line = 0;
  int len, total_len = 0;
  THELIST *curr_name;
...
short extract_position(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  long true_line = 0;
  long col = 0;
...
short extract_prefix(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  /*
  ...
  if (strcmp((char *) itemargs, "") == 0) {
    if (CURRENT_VIEW->prefix == PREFIX_OFF) {
      ...
      item_values[1].len = 3;
      item_values[2].value = (uchar *) "";      /* this set to empty deliberately */
      item_values[2].len = 0;
      return 1;
    }
    if ((CURRENT_VIEW->prefix & PREFIX_STATUS_MASK) == PREFIX_ON) {
      ...
      item_values[1].value = (uchar *) "NULLS";
      item_values[1].len = 5;
    }
    if ((CURRENT_VIEW->prefix & PREFIX_LOCATION_MASK) == PREFIX_LEFT) {
      item_values[2].value = (uchar *) "LEFT";
      item_values[2].len = 4;
    } else {
      item_values[2].value = (uchar *) "RIGHT";
      item_values[2].len = 5;
    }
...
short extract_printer(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  item_values[1].value = (uchar *) spooler_name;
  item_values[1].len = strlen((char *) spooler_name);
  return number_variables;
}

short extract_profile(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (local_prf == NULL) {
    item_values[1].value = (uchar *) "";
    item_values[1].len = 0;
  } else {
    item_values[1].value = (uchar *) local_prf;
    item_values[1].len = strlen((char *) local_prf);
  }
  return number_variables;
}

short extract_pscreen(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_reprofile(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(REPROFILEx, 1);
}

short extract_readonly(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (READONLYx == READONLY_FORCE) {
    item_values[1].value = (uchar *) "FORCE";
    item_values[1].len = 5;
...
short extract_readv(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  int key = 0;
...
short extract_reserved(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  bool line_numbers_only = TRUE;
  RESERVED *curr_rsrvd;
  uchar *tmpbuf = NULL, *attr_string = NULL;
  short rc = RC_OK;
  short y = 0, x = 0;
  ...
  while (curr_rsrvd != NULL) {
    if (line_numbers_only) {
      ...
      tmpbuf = (uchar *) malloc(sizeof(uchar) * (strlen((char *) attr_string) + strlen((char *) curr_rsrvd->line) + strlen((char *) curr_rsrvd->spec) + 13));
...
short extract_rexxhalt(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (COMMANDCALLSx == 0) {
    item_values[1].value = (uchar *) "OFF";
    item_values[1].len = 3;
  } else {
...
short extract_rexxoutput(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (CAPREXXOUTx) {
    item_values[1].value = (uchar *) "FILE";
    item_values[1].len = 4;
  } else {
    item_values[1].value = (uchar *) "DISPLAY";
    item_values[1].len = 7;
  }
...
short extract_rightedge_function(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  unsigned short x = 0;
...
short extract_ring(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  VIEW_DETAILS *curr;
    ...
    if (process_view) {
      ...
      for (i = 0; i < display_screens; i++) {
        if (SCREEN_VIEW(i) == curr) {
          ...
          get_current_position((uchar) i, &lineno, &col);
        }
      }
...
short extract_scale(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_scope(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (CURRENT_VIEW->scope_all) {
    item_values[1].value = (uchar *) "ALL";
    item_values[1].len = 3;
  } else {
    item_values[1].value = (uchar *) "DISPLAY";
    item_values[1].len = 7;
...
short extract_screen(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  sprintf((char *) query_num1, "%d", display_screens);
  item_values[1].value = query_num1;
  item_values[1].len = strlen((char *) query_num1);
  if (horizontal) {
    item_values[2].value = (uchar *) "HORIZONTAL";
    item_values[2].len = 10;
  } else {
    item_values[2].value = (uchar *) "VERTICAL";
    item_values[2].len = 8;
  }
  return number_variables;
}

short extract_select(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  LINE *curr = NULL;
  unsigned short x = 0;
  ...
  item_values[2].value = query_num2;
  item_values[3].len = sprintf((char *) query_num3, "%d", MAX_SELECT_LEVEL);
  item_values[3].value = query_num3;
  return number_variables;
}

short extract_shadow(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_VIEW->shadow, 1);
}

short extract_shadow_function(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  bool bool_flag = FALSE;
  unsigned short y = 0;
...
short extract_shift_function(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  int shift = 0;
...
short extract_showkey(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  int key = 0;

  if (strcmp((char *) itemargs, "") == 0 || ((key = find_key_name(itemargs)) == -1)) {
    item_values[1].value = (uchar *) "INVALID KEY";
    item_values[1].len = strlen((char *) item_values[1].value);
  } else {
...
short extract_size(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  sprintf((char *) query_num1, "%ld", CURRENT_FILE->number_lines);
  item_values[1].value = query_num1;
  item_values[1].len = strlen((char *) query_num1);
  return number_variables;
...
short extract_slk(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  int off = 0;
  int item;

  if (blank_field(itemargs)) {
    if (max_slk_labels == 0 && query_type != QUERY_MODIFY) {
      item_values[1].value = (uchar *) "UNAVAILABLE";
    ...
    if (valid_positive_integer(itemargs)) {
      ...
      if (item < 1) {
        display_error(5, (uchar *) itemargs, FALSE);
        return EXTRACT_ARG_ERROR;
      }
      if (item > max_slk_labels) {
        display_error(6, (uchar *) itemargs, FALSE);
        return EXTRACT_ARG_ERROR;
      }
      if (query_type == QUERY_QUERY) {
      ...
      display_error(4, (uchar *) itemargs, FALSE);
      number_variables = EXTRACT_ARG_ERROR;
    }
  }
  return (number_variables);
...
short extract_spacechar_function(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  uchar cursor_char = 0;
  ...
  cursor_char = (uchar) (winch(CURRENT_WINDOW) & A_CHARTEXT);
  return set_boolean_value((bool) (cursor_char == ' '), (short) 1);
}

short extract_statopt(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  short rc = RC_OK;
  int i = 0;
  int off = 0;
  ushort save_msgline_rows = CURRENT_VIEW->msgline_rows;
...
short extract_statusline(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  switch (STATUSLINEx) {
    case 'B':
...
short extract_stay(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_VIEW->stay, 1);
}

#define SYN_PARAMS  4

short extract_synelem(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  ...
  if (num_params == 0 || equal((uchar *) "cursor", word[0], 1)) {
    /*
    ...
    if (CURRENT_VIEW->current_window != WINDOW_FILEAREA || !curses_started) {
      item_values[1].value = (uchar *) "UNKNOWN";
      item_values[1].len = 4;
      return (number_variables);
    }
    /*
  ...
  } else if (equal((uchar *) "file", word[0], 1)) {
    if (num_params > 3) {
      ...
      /*
       * Now have two numbers, do they specify a file position that is currently viewable
       */
      ...
      x = (long) col - (long) CURRENT_VIEW->verify_col;
    ...
    if (num_params > 2) {
      ...
      /*
       * Now have two numbers, do they specify a position that is currently viewable
       */
  ...
  switch (syntax_element) {
    ...
    case THE_SYNTAX_COMMENT:
      item_values[1].value = (uchar *) "COMMENT";
      item_values[1].len = 7;
      break;
    case THE_SYNTAX_FUNCTION:
      item_values[1].value = (uchar *) "FUNCTION";
      item_values[1].len = 8;
      break;
    case THE_SYNTAX_HEADER:
      item_values[1].value = (uchar *) "HEADER";
      item_values[1].len = 6;
      break;
    case THE_SYNTAX_INCOMPLETESTRING:
      item_values[1].value = (uchar *) "INCOMPLETESTRING";
      item_values[1].len = 16;
      break;
    case THE_SYNTAX_KEYWORD:
      item_values[1].value = (uchar *) "KEYWORD";
      item_values[1].len = 7;
      break;
    case THE_SYNTAX_LABEL:
      item_values[1].value = (uchar *) "LABEL";
      item_values[1].len = 5;
      break;
    case THE_SYNTAX_MARKUP:
      item_values[1].value = (uchar *) "MARKUP";
      item_values[1].len = 6;
      break;
    case THE_SYNTAX_MATCH:
      item_values[1].value = (uchar *) "MATCH";
      item_values[1].len = 5;
      break;
    case THE_SYNTAX_NUMBER:
      item_values[1].value = (uchar *) "NUMBER";
      item_values[1].len = 6;
      break;
    case THE_SYNTAX_POSTCOMPARE:
      item_values[1].value = (uchar *) "POSTCOMPARE";
      item_values[1].len = 11;
      break;
    case THE_SYNTAX_PREPROCESSOR:
      item_values[1].value = (uchar *) "PREPROCESSOR";
      item_values[1].len = 12;
      break;
    case THE_SYNTAX_STRING:
      item_values[1].value = (uchar *) "STRING";
      item_values[1].len = 6;
      break;
    default:
      item_values[1].value = (uchar *) "UNKNOWN";
...
short extract_synonym(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  int off = 0;
  uchar *ptr = NULL;
  DEFINE *curr;
...
short extract_tabkey(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (tabkey_overwrite == 'T') {
    item_values[1].value = (uchar *) "TAB";
    item_values[1].len = 3;
  } else {
    item_values[1].value = (uchar *) "CHARACTER";
    item_values[1].len = 9;
  }
  if (tabkey_insert == 'T') {
    item_values[2].value = (uchar *) "TAB";
    item_values[2].len = 3;
  } else {
    item_values[2].value = (uchar *) "CHARACTER";
    item_values[2].len = 9;
  }
  return number_variables;
}

short extract_tabline(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_tabs(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  int i = 0;
...
short extract_tabsin(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_tabsout(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_targetsave(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  int len = 0;
...
short extract_terminal(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_thighlight(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_VIEW->thighlight_on, 1);
}

short extract_timecheck(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_FILE->timecheck, 1);
}

short extract_tof(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value((bool) CURRENT_TOF, 1);
}

short extract_tofeof(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_VIEW->tofeof, 1);
}

short extract_tof_function(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_boolean_value((bool) (FOCUS_TOF && CURRENT_VIEW->current_window != WINDOW_COMMAND), (short) 1);
}

short extract_topedge_function(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  unsigned short y = 0;
...
short extract_trailing(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  switch (CURRENT_FILE->trailing) {
    ...
    case TRAILING_SINGLE:
      item_values[1].value = (uchar *) "SINGLE";
      item_values[1].len = 6;
      break;
    case TRAILING_EMPTY:
      item_values[1].value = (uchar *) "EMPTY";
      item_values[1].len = 5;
      break;
    case TRAILING_REMOVE:
      item_values[1].value = (uchar *) "REMOVE";
      item_values[1].len = 6;
      break;
    default:
      break;
...
short extract_typeahead(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(TYPEAHEADx, 1);
}

short extract_ui(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_undoing(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_FILE->undoing, 1);
}

short extract_untaa(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(UNTAAx, 1);
}

short extract_utf8(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(0, 1);
}

short extract_verify(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_vershift(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  sprintf((char *) query_rsrvd, "%ld", (long) CURRENT_VIEW->verify_col - (long) CURRENT_VIEW->verify_start);
  item_values[1].value = query_rsrvd;
  item_values[1].len = strlen((char *) query_rsrvd);
  return number_variables;
}

short extract_verone_function(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_boolean_value((bool) (CURRENT_VIEW->verify_col == 1), (short) 1);
}

short extract_version(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  item_values[1].value = (uchar *) "THE";
  item_values[1].len = 3;
  item_values[2].value = (uchar *) the_version;
  item_values[2].len = strlen((char *) the_version);
  item_values[3].value = (uchar *) "???";
  item_values[3].value = (uchar *) "UNIX";
  item_values[3].len = strlen((char *) item_values[3].value);
  item_values[4].value = (uchar *) the_release;
  item_values[4].len = strlen((char *) item_values[4].value);
...
short extract_width(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
...
short extract_word(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  if (CURRENT_VIEW->word == 'A') {
    item_values[1].value = (uchar *) "ALPHANUM";
    item_values[1].len = 8;
  } else {
    item_values[1].value = (uchar *) "NONBLANK";
    item_values[1].len = 8;
  }
  return number_variables;
}

short extract_wordwrap(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_VIEW->wordwrap, 1);
}

short extract_wrap(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
  return set_on_off_value(CURRENT_VIEW->wrap, 1);
}

short extract_zone(short number_variables, short itemno, uchar *itemargs, uchar query_type, long argc, uchar *arg, long arglen) {
