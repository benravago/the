...
short extract_parser(short number_variables, short itemno, CHARTYPE * itemargs, CHARTYPE query_type, LINETYPE argc, CHARTYPE * arg, LINETYPE arglen) {
  ...
  if (itemargs == NULL || blank_field(itemargs)
    ...
    for (curr = first_parser; curr != NULL; curr = curr->next) {
      sprintf((DEFCHAR *) query_rsrvd, "%s%s %s", (query_type == QUERY_QUERY) ? (DEFCHAR *) "parser " : "", curr->parser_name, curr->filename);
    ...
    if (query_type == QUERY_QUERY) {
      sprintf((DEFCHAR *) query_rsrvd, "%s%s %s", (query_type == QUERY_QUERY) ? (DEFCHAR *) "parser " : "", itemargs, ptr_filename);
      display_error(0, query_rsrvd, TRUE);
    } else {
  ...
  if (number_variables >= 0) {
    ...
    if (CURRENT_FILE->first_ppc == NULL)
      ...
      for (;;) {
        ...
        if (!(IN_SCOPE(CURRENT_VIEW, curr)
              || CURRENT_VIEW->scope_all || curr_ppc->ppc_shadow_line)) {
          curr_ppc = curr_ppc->next;
          continue;
        }
        /*
...
short extract_reserved(short number_variables, short itemno, CHARTYPE * itemargs, CHARTYPE query_type, LINETYPE argc, CHARTYPE * arg, LINETYPE arglen) {
  ...
  if (query_type == QUERY_EXTRACT && strcmp((DEFCHAR *) itemargs, "*") == 0)
  ...
  for (j = 0; j < number_of_files;) {
    ...
    if (process_view) {
      ...
      if (compatible_look == COMPAT_XEDIT)
        sprintf((DEFCHAR *) query_rsrvd, "%s%s Size=%ld Line=%ld Col=%ld Alt=%d,%d", curr->file_for_view->fpath, curr->file_for_view->fname, curr->file_for_view->number_lines, lineno, col, curr->file_for_view->autosave_alt, curr->file_for_view->save_alt);
      else
        sprintf((DEFCHAR *) query_rsrvd, "%s%s Line=%ld Col=%ld Size=%ld Alt=%d,%d", curr->file_for_view->fpath, curr->file_for_view->fname, lineno, col, curr->file_for_view->number_lines, curr->file_for_view->autosave_alt, curr->file_for_view->save_alt);
      if (query_type == QUERY_QUERY) {
        display_error(0, query_rsrvd, TRUE);
      } else {
        number_variables++;
...
short extract_showkey(short number_variables, short itemno, CHARTYPE * itemargs, CHARTYPE query_type, LINETYPE argc, CHARTYPE * arg, LINETYPE arglen) {
  ...
  if (strcmp((DEFCHAR *) itemargs, "") == 0 || ((key = find_key_name(itemargs)) == -1)) {
...
short extract_statopt(short number_variables, short itemno, CHARTYPE * itemargs, CHARTYPE query_type, LINETYPE argc, CHARTYPE * arg, LINETYPE arglen) {
  ...
  if (itemargs == NULL || blank_field(itemargs)
    ...
    for (curr = first_option; curr != NULL; curr = curr->next) {
      sprintf((DEFCHAR *) query_rsrvd, "%sON %s %d %d %s", (query_type == QUERY_QUERY) ? (DEFCHAR *) "statopt " : "", curr->name, curr->select + 1 + STATAREA_OFFSET, curr->save_select, (DEFCHAR *) ((curr->line != NULL) ? (DEFCHAR *) curr->line : ""));
    ...
    if (curr) {
       ...
       * We found it
       */
      sprintf((DEFCHAR *) query_rsrvd, "%sON %s %d %d %s", (query_type == QUERY_QUERY) ? (DEFCHAR *) "statopt " : "", curr->name, curr->select, curr->save_select, (DEFCHAR *) ((curr->line != NULL) ? (DEFCHAR *) curr->line : ""));
    } else {
      /*
       * We didn't find it
       */
      sprintf((DEFCHAR *) query_rsrvd, "%sOFF %s", (query_type == QUERY_QUERY) ? (DEFCHAR *) "statopt " : "", itemargs);
    }
    if (query_type == QUERY_QUERY)
      display_error(0, query_rsrvd, TRUE);
    else {
...
#define SYN_PARAMS  4
  ...
  if (num_params == 0 || equal((CHARTYPE *) "cursor", word[0], 1)) {
    /*
