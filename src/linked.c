  ...
  if ((next = (THELIST*) malloc(size)) != (THELIST*) NULL) {
    ...
    if (curr == NULL) {
      if (first == NULL) {
        ...
        /*
         * Insert this entry before first.
         * Calling routine must reset first to returned pointer.
         */
        next->next = first;
        next->prev = NULL;
        first->prev = next;
...
LINE* lll_locate(LINE* first, char* value) {
  LINE* curr = NULL;
...
PARSER_DETAILS* parserll_find(PARSER_DETAILS* first, char* name) {
  PARSER_DETAILS* curr = NULL;
  short i = 0;
...
PARSER_MAPPING* mappingll_find(PARSER_MAPPING* first, char* filemask, char* magic_number) {
  PARSER_MAPPING* curr = NULL;
  short i = 0;
