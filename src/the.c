...
char *profile_command_line = NULL;

char *target_buffer = NULL;        /* used in get_item_values() */
...
char fifo_name[MAX_FILE_NAME + 1];
char pid_name[MAX_FILE_NAME + 1];

char *rexxoutname = "REXX.$$$";
char *keyfilename = "KEY$$$.$$$";

char rexx_pathname[MAX_FILE_NAME + 1];
char rexx_filename[10];

char *dirfilename = "DIR.DIR";
...
char key_pathname[MAX_FILE_NAME + 1];
char key_filename[15];
...
char dir_files[MAX_FILE_NAME + 1]; /* for dir and ls commands */

char rexx_macro_name[MAX_FILE_NAME + 1];   /* current rexx macro name */
char rexx_macro_parameters[MAX_FILE_NAME + 1];     /* current rexx macro parameters */

char the_home_dir[MAX_FILE_NAME + 1];
char the_help_file[MAX_FILE_NAME + 1];

char the_macro_path[MAX_FILE_NAME + 1];
char the_macro_path_buf[MAX_FILE_NAME + 1];
...
char spooler_name[MAX_FILE_NAME + 1];
...
int main(int argc, char *argv[]) {
  ...
  if ((envptr = getenv("HOME")) != NULL) {
    if (((envptr == NULL) ? 0 : strlen((char *) envptr)) > MAX_FILE_NAME) {
      cleanup();
      display_error(7, envptr, FALSE);
      return (7);
    }
    strcpy(user_home_dir, envptr);
  } else {
  ...
  if ((envptr = getenv("THE_HOME_DIR")) != NULL) {
    ...
    strcpy(the_home_dir, user_home_dir);
  ...
  while ((c = getopt(my_argc, my_argv, mygetopt_opts)) != EOF) {
    switch ((char) c) {
      ...
      case 'l':                /* set current line on startup */
        ...
        if (startup_line < 0L) {
          cleanup();
          display_error(5, "startup line MUST be > 0", FALSE);
          return (4);
        }
        break;
      ...
      case 'c':                /* set current column on startup */
        ...
        if (startup_column == 0) {
          cleanup();
          display_error(5, "startup column MUST be > 0", FALSE);
          return (4);
        }
        break;
      ...
      case 'p':                /* profile file name */
        if ((specified_prf = (char*) malloc((strlen(optarg) + 1) * sizeof(char))) == NULL) {
          cleanup();
      ...
      case 'w':                /* width of line */
        if ((rc = valid_positive_integer_against_maximum((uchar*)optarg, MAX_WIDTH_NUM)) != 0) {
          ...
          display_error(rc, mygetopt_opts, FALSE);
          return (4);
        }
        max_line_length = atol(optarg);
        if (max_line_length < 10L) {
          cleanup();
          display_error(5, "- width MUST be >= 10", FALSE);
          return (4);
        }
        if (max_line_length > MAX_WIDTH_NUM) {
          ...
          /* safe to use mygetopt_opts as we are bailing out */
          sprintf(mygetopt_opts, "- width MUST be <= %ld", MAX_WIDTH_NUM);
          display_error(6, mygetopt_opts, FALSE);
          return (5);
        }
        break;
      ...
      case 'u':                /* display length */
        ...
        if (display_length == 0) {
          cleanup();
          display_error(5, "- display length MUST be > 0", FALSE);
          return (4);
        }
        break;
      ...
      case '1':                /* allow single instances */
         ...
         */
        if (optarg == NULL) {
          strcpy(fifo_name, user_home_dir);
          strcat(fifo_name, THE_FIFO_FILE);
          strcpy(pid_name, user_home_dir);
          strcat(pid_name, THE_PID_FILE);
          break;
        }
  ...
  if (display_length > 0 && display_length > max_line_length) {
    cleanup();
    display_error(6, "- width MUST be >= display length", FALSE);
    return (8);
  }
  /*
  ...
  if (specified_prf && *specified_prf == '-' && batch_only) {
    ...
    if ((stdinprofile = strdup(tmpnam(NULL))) == NULL) {   // thetmpnam ( "PRF" )
      cleanup();
      return (31);
    }
   ...
   */
  if ((pre_rec = (char*) malloc((MAX_PREFIX_WIDTH + 1) * sizeof(char))) == NULL) {
    cleanup();
    display_error(30, "", FALSE);
    return (16);
  }
   ...
   */
  if ((tempfilename = strdup(tmpnam(NULL))) == NULL) {     // thetmpnam ( "TMP" )
    cleanup();
    return (31);
  }
  /*
  ...
  if (batch_only) {
    ...
    while (current_file_name != NULL) {
      if ((rc = get_file(current_file_name->line)) != RC_OK) {
        cleanup();
        if (rc == RC_DISK_FULL) {
          display_error(57, "...probably", FALSE);
        }
        return (21);
      }
    ...
    if (number_of_files != 0) {
      sprintf(rec, "%ld", number_of_files);
  ...
  if (create_statusline_window() != RC_OK) {
    cleanup();
    display_error(0, "creating status line window", FALSE);
    return (23);
  }
  if (create_filetabs_window() != RC_OK) {
    cleanup();
    display_error(0, "creating filetabs window", FALSE);
    return (23);
  }
  /*
   ...
   */
  Statopt("ON NBFILE.1 13 0 Files=");
  Statopt("ON WIDTH.1 23 0 Width=");
  /*
   * Read each file into memory and apply the profile file to each of the files.
   */
  current_file_name = first_file_name;
  while (current_file_name != NULL) {
    rc = EditFile(current_file_name->line, TRUE);
    if (rc != RC_OK) {
      cleanup();
      return (24);
    }
...
static void init_signals(void) {
  ...
  signal(SIGABRT, handle_signal);  // 6
  signal(SIGBUS, handle_signal);   // 7
  signal(SIGFPE, handle_signal);   // 8
  signal(SIGSEGV, handle_signal);  // 11
  signal(SIGPIPE, SIG_IGN);        // 13
  signal(SIGTERM, handle_signal);  // 15
  signal(SIGWINCH, handle_signal); // 28
...
int setup_profile_files(char * specified_prf) {
  int rc = RC_OK;
  char *envptr = NULL;
  short errnum = 0;
   ...
   */
  if (specified_prf != NULL) {
    if ((local_prf = (char*) malloc((MAX_FILE_NAME + 1) * sizeof(char))) == NULL) {
  ...
  /*
   * Lastly try for a default profile file in THE_HOME_DIR if set or directory where THE executable lives
   */
...
static void display_info(char * argv0) {
...
int allocate_working_memory(void) {
   ...
   */
  if (rec == NULL) {
    rec = (char*) malloc((max_line_length + 5) * sizeof(char));
  } else {
    rec = (char*) realloc(rec, (max_line_length + 5) * sizeof(char));
  }
  if (rec == NULL) {
    return (10);
  }
  ...
  if (trec == NULL) {
    trec = (char*) malloc(trec_len * sizeof(char));
  } else {
    trec = (char*) realloc(trec, trec_len * sizeof(char));
  }
  if (trec == NULL) {
    return (11);
  }
  ...
  if (brec == NULL) {
    brec = (char*) malloc(brec_len * sizeof(char));
  } else {
    brec = (char*) realloc(brec, brec_len * sizeof(char));
  }
  if (brec == NULL) {
    return (11);
  }
   ...
   */
  if (cmd_rec == NULL) {
    cmd_rec = (char*) malloc((max_line_length + 2) * sizeof(char));
  } else {
    cmd_rec = (char*) realloc(cmd_rec, (max_line_length + 2) * sizeof(char));
  }
  if (cmd_rec == NULL) {
    return (12);
  }
  ...
  /*
   * Allocate some memory to profile_command_line. (only really need to do this if NO Rexx support).
   */
  if (profile_command_line == NULL) {
    profile_command_line = (char*) malloc((max_line_length + 2) * sizeof(char));
  } else {
    profile_command_line = (char*) realloc(profile_command_line, (max_line_length + 2) * sizeof(char));
  }
  if (profile_command_line == NULL) {
    return (17);
  }
...
void cleanup(void) {
  ...
  if (curses_started) {
    if (error_on_screen && error_window != NULL) {
      display_error(0, HIT_ANY_KEY, FALSE);
