...
char_t *the_copyright = (char_t *) THE_COPYRIGHT;

char_t *term_name;            /* $TERM value */
...
int main(int argc, char *argv[]) {
  ...
  if ((envptr = getenv("TERM")) != NULL) {
    term_name = (char_t *) envptr;
  } else {
    term_name = (char_t *) "default";
  }
  /*
  ...
  while ((c = getopt(my_argc, my_argv, mygetopt_opts)) != EOF) {
    switch ((char) c) {
      ...
      default:
        cleanup();
        display_error(0, (char_t *) "Invalid command line switch", FALSE);
        return (0);
        break;
    }
  }
  ...
  if (optind < my_argc) {
    ...
    if ((current_file_name = add_LINE(first_file_name, current_file_name, (char_t *) CURRENT_DIR, strlen((char *) CURRENT_DIR), 0, TRUE)) == NULL) {
      cleanup();
      display_error(30, (char_t *) "", FALSE);
      return (7);
    }
  ...
  if (specified_prf && *specified_prf == '-' && batch_only) {
    FILE *fp = NULL;
    int num = 0;
    if ((stdinprofile = (char_t *) tmpname("PRF")) == NULL) {
      cleanup();
      return (31);
    }
   ...
   */
  if ((tempfilename = (char_t *) tmpname("TMP")) == NULL) {
    cleanup();
    return (31);
  }
  /*
