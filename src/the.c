...
static char RCSid[] = "$Id: the.c,v 1.3 1999/07/31 07:38:26 mark Exp mark $";
...
/*--------------------------- global data -----------------------------*/
   ...
   CHARTYPE *the_version = (CHARTYPE *)"3.0";
   CHARTYPE *the_release = (CHARTYPE *)"16-January-2000";
   CHARTYPE *the_copyright = (CHARTYPE *)"Copyright 1991-2000 Mark Hessling";
...
/*--------------------------- processing ------------------------------*/
  ...
  /*---------------------------------------------------------------------*/
#if defined(XCURSES) & PDC_BUILD >= 2401
   while ((c = my_getopt(argc,argv,"qk::sSbmnrl:c:p:w:a:u:X:h?")) != EOF)
#else
   while ((c = my_getopt(argc,argv,"qk::sSbmnrl:c:p:w:a:u:h?")) != EOF)
#endif
   {
      switch((char)c)
      {
...
/*--------------------------- processing ------------------------------*/
    ...
    */
   argv = (*the_malloc)( max_args * max_args );
   if (argv == NULL)
      return NULL;
   /*
