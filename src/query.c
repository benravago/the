...
short find_query_item(CHARTYPE * item_name, int len, CHARTYPE * query_type) {
  ...
  rc = search_query_item_array(query_item, number_query_item(), sizeof(QUERY_ITEM), (DEFCHAR *) item_name, len);
  if (rc != (-1))
...
short extract_colour_settings(short itemno, CHARTYPE * buffer, CHARTYPE query_type, CHARTYPE * params, bool us, bool isecolour) {
  ...
  for (i = start; i < end; i++) {
    ...
    sprintf((DEFCHAR *) query_rsrvd, "%s%s %s", (query_type == QUERY_QUERY) ? (DEFCHAR *) ptr : "", area, attr_string);
...
short extract_autocolour_settings(short itemno, CHARTYPE * buffer, CHARTYPE query_type, CHARTYPE * params, bool us) {
  ...
  if (blank_field(params)
    ...
    for (curr = first_parser_mapping; curr != NULL; curr = curr->next) {
      sprintf((DEFCHAR *) query_rsrvd, "%s%s%s %s%s", (query_type == QUERY_QUERY) ? (DEFCHAR *) ptr : "", (curr->filemask) ? (DEFCHAR *) curr->filemask : "", (curr->magic_number) ? (DEFCHAR *) curr->magic_number : "", (DEFCHAR *) curr->parser->parser_name,
     ...
     */
    for (curr = first_parser_mapping; curr != NULL; curr = curr->next) {
      if (curr->filemask && strcmp((DEFCHAR *) params, (DEFCHAR *) curr->filemask) == 0) {
        ptr_mask = curr->filemask;
        ptr_magic = (CHARTYPE *) "";
        found = TRUE;
        break;
      }
      if (curr->magic_number && strcmp((DEFCHAR *) params, (DEFCHAR *) curr->magic_number) == 0) {
    ...
    if (query_type == QUERY_QUERY) {
      sprintf((DEFCHAR *) query_rsrvd, "%s%s %s%s", (query_type == QUERY_QUERY) ? (DEFCHAR *) ptr : "", ptr_mask, ptr_parser, ptr_magic);
      display_error(0, query_rsrvd, TRUE);
    } else {
