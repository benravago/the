...
/*--------------------------- processing ------------------------------*/
 ...
 if (strcmp("XXYYZZ",(DEFCHAR *)temp_cmd) == 0)
 {
    ...
    while(rc == 0)
    {
       ...
       {
          fprintf(stderr,"<");
          memcpy(temp_cmd,shv.shvname.strptr,shv.shvname.strlength);
          temp_cmd[shv.shvname.strlength] = '\0';
          fprintf(stderr,"%s>",temp_cmd);

          fprintf(stderr," <");
          memcpy(temp_cmd,shv.shvvalue.strptr,shv.shvvalue.strlength);
          temp_cmd[shv.shvvalue.strlength] = '\0';
          fprintf(stderr,"%s>",temp_cmd);
...
/*---------------------------------------------------------------------*/
    ...
    switch(function_item[rc].item_number)
    {
       ...
       default:
            ...
            if ((Argv == NULL) || (Argc == 0)) /* FGC */
               num_vars = get_item_values(0,function_item[rc].item_number,
 ...
 else
 {
    ...
    if ((Argv == NULL) || (Argc == 0)) /* FGC */
       num_vars = get_item_values(1,query_item[rc].item_number,
             (CHARTYPE *)"",QUERY_FUNCTION,(LINETYPE)Argc,NULL,0L);
    else
       num_vars = get_item_values(1,query_item[rc].item_number,
             (CHARTYPE *)"",QUERY_FUNCTION,(LINETYPE)Argc,
...
#else
    Retstr->strptr = (RXSTRING_STRPTR_TYPE)malloc(item_values[item_index].len);
#endif
 ...
 memcpy(Retstr->strptr,item_values[item_index].value,
                       item_values[item_index].len);
...
/*--------------------------- processing ------------------------------*/
 ...
 for (i=0; i<number_query_item(); i++)
 {
    ...
    {
       sprintf(buf,"%s.%d",query_item[i].name,j);
       rc = RexxDeregisterFunction((RDF_ARG0_TYPE)buf);
    }
 }
...
/***********************************************************************/
{
 ...
 rc = RexxVariablePool(&shv);            /* get the next variable */
 if (rc == 0)
/*  version_buffer[shv.shvvalue.strlength] = '\0'; */
