...
static char RCSid[] = "$Id: comm1.c,v 1.1 1999/06/25 06:11:56 mark Exp mark $";
...
/*--------------------------- local data ------------------------------*/
 ...
 CHARTYPE save_number_of_files=number_of_files;
 register int i=0;
/*--------------------------- processing ------------------------------*/
#ifdef THE_TRACE
...
/*---------------------------------------------------------------------*/
 ...
 if (number_of_files > 0)
   {
    sprintf((DEFCHAR *)temp_cmd,"%d file(s) remain with outstanding changes",number_of_files);
    ...
    switch(state)
      {
       ...
       case CURSOR_ESCREEN:
            ...
            if (equal((CHARTYPE *)"left",word[idx],4))
              {
               ...
               rc = THEcursor_left(escreen,FALSE);
               time_to_leave = TRUE;
               break;
              }
            if (equal((CHARTYPE *)"right",word[idx],5))
              {
               ...
               rc = THEcursor_right(escreen,FALSE);
               time_to_leave = TRUE;
               break;
              }
            if (equal((CHARTYPE *)"up",word[idx],2))
            ...
            if (strcmp((DEFCHAR *)word[idx],"") == 0)
              {
               if (!escreen) /* MH 0501 - was if (escreen) */
                  col = 1;
               else
            ...
            rc = THEcursor_move((escreen)?FALSE:TRUE,row,col); /* MH 0501 - was just escreen */
            time_to_leave = TRUE;
            break;
       case CURSOR_KEDIT:
            if (equal((CHARTYPE *)"left",word[idx],4))
