 ...
 * Copyright (C) 1991-1998 Mark Hessling
 *
...
static char RCSid[] = "$Id: commutil.c,v 1.1 1999/06/25 06:11:56 mark Exp mark $";
...
static void save_last_command();
 ...
 CHARTYPE _THE_FAR last_change_command[MAX_COMMAND_LENGTH];
 CHARTYPE _THE_FAR last_target[MAX_COMMAND_LENGTH];
 ...
 while(curr != (DEFINE *)NULL)
   {
    if (key == curr->def_funkey)
      {
       rc = append_define(&first_save,&last_save,key,curr->def_command,curr->def_params,curr->pcode,curr->pcode_len);
       if (rc != RC_OK)
         {
#ifdef THE_TRACE
          trace_return();
 ...
 if (instore)
   {
    if (rexx_support)
      {
       remove_define(first,last,key_value);
       rc = append_define(first,last,key_value,(-1),commands,NULL,0);
#ifdef THE_TRACE
       trace_return();
#endif
       return(rc);
    ...
    for (j=0;j<num_commands;j++)
      {
       ...
       rc = append_define(first,last,key_value,cmd_nr,cl_param,NULL,0);
       if (rc != RC_OK)
          break;
      }
   }
...
/***********************************************************************/
#ifdef HAVE_PROTO
short append_define(DEFINE **first,DEFINE **last,int key_value,short cmd,CHARTYPE *prm,CHARTYPE *pcode,int pcode_len)
#else
short append_define(first,last,key_value,cmd,prm,pcode,pcode_len)
DEFINE **first,**last;
int key_value;
short cmd;
CHARTYPE *prm;
